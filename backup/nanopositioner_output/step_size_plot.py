...
def plot_mirror_error_posic(last_keyence_df):
    """
    Same repeatability calculation as plot_mirror_error but using posic_encoder_mm.
    Plots error by subtracting posic_encoder_mm values from opposite ends of the dataframe.
    X-axis shows the magnetic_raw_mm value of the outer row, rounded to nearest tenth.
    """
    ...
    for i in range(mid):
        outer_idx = n - 1 - i  # row N, N-1, N-2...
        inner_idx = i           # row 0, 1, 2...
        
        outer_row = last_keyence_df.iloc[outer_idx]
        inner_row = last_keyence_df.iloc[inner_idx]
        
        error = outer_row['posic_encoder_mm'] - inner_row['posic_encoder_mm']
        x_val = round(outer_row['magnetic_raw_mm'], 1)
    ...
    ax.bar(x_positions, errors, color='seagreen', edgecolor='black', alpha=0.8)
    ax.axhline(0, color='black', linewidth=0.8, linestyle='--')
    
    ax.set_xticks(x_positions)
    ax.set_xticklabels(x_labels, rotation=45, ha='right')
    ax.set_xlabel('Magnetic Raw Position (mm)', fontsize=12)
    ax.set_ylabel('Posic Encoder Error (mm)', fontsize=12)
    ax.set_title('Repeatability Error: Outer vs Inner Posic Encoder Readings', fontsize=14)
    ax.grid(True, axis='y', alpha=0.3)
    
    plt.tight_layout()
    
    output_path = os.path.join(graphs_dir, f'mirror_error_posic_v{version}.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"Posic encoder mirror error plot saved to: {output_path}")
...
df_merged['posic_encoder_mm'] = df_merged['posic_encoder_mm'] / 1000000 * 19.53125 * (-1)
...
# errors_posic, x_labels_posic = plot_mirror_error_posic(last_keyence_df)  # <-- add this line
