...
def plot_keyence_vs_magnetic(df_merged, version):
    ...
    valid_mask = df_merged['displacement_mm'].notna() & df_merged['magnetic_raw_filtered'].notna()
    
    keyence_data = df_merged.loc[valid_mask, 'displacement_mm'].values
    ...
    ax.set_ylabel('Magnetic Filtered Displacement (mm)', fontsize=12)
    ax.set_title(f'Keyence vs Magnetic Sensor Comparison (Version {version})', fontsize=14)
    ...
    # Save the figure
    output_path = os.path.join(graphs_dir, f'keyence_vs_magnetic_v{version}.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"\nKeyence vs Magnetic correlation plot saved to: {output_path}")
...
df_arduino['magnetic_raw_filtered'] = df_arduino['magnetic_raw_mm']
